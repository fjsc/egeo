  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => StTagInputComponent),
      multi: true
    },
    {
...
export class StTagInputComponent
  ...
  get isValidInput(): boolean {
    const isForbidden =
      this.forbiddenValues.length &&
      this.forbiddenValues.indexOf(this.innerInputContent) > -1;
    const isDuplicated = this.items.indexOf(this.innerInputContent) !== -1;
    const matchedPattern = this.regularExpression
      ? this._regularExp.test(this.innerInputContent)
      : true;
    return this.innerInputContent.length
      ? !isForbidden && !isDuplicated && matchedPattern
      : true;
  }
  ...
  showError(): boolean {
    return (
      ...
      (!this.isPristine || this.forceValidations) &&
      !this._focus &&
      !this.disabled
    );
  }
  ...
  private showAutocompleteMenu(): void {
    if (
      this.withAutocomplete &&
      !this.expandedMenu &&
      this.charsToShowAutocompleteList <= this.innerInputContent.length
    ) {
